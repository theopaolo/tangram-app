export let PIECES_DATA = {
     1: {
      color: '#1a5435',
      rvb: '26, 84, 53',
      cmjn: '95, 37, 91, 38',
      color_name: 'Vert forêt scintillante',
      titre_artwork: 'Tutti Quanti',
      artiste: 'John M. Armleder',
      date: '2018',
      story: 'Pour cette œuvre, l’artiste John Armleder a utilisé de nombreuses techniques : de la peinture, des feuilles, des paillettes, des faux cheveux, une poupée, ... Il a ressemblé ces éléments sur sa toile posée au sol, qu’il a ensuite relevée. Ce mouvement a permis aux différents matériaux de se mélanger et de créer cet effet de coulure.',
      dapres_toi: 'D’après toi, cette œuvre représente ?',
      dapres: [
        'Un lendemain de Noël',
        'Une scène de bataille au magasin de jouet',
        'Un rêve de Drag Queen'
      ],
      images: [
        {
          src: "/images/chromo_5.jpg",
          credits: "© John M. Armleder. Photographie : Annik Wetter."
        },
      ],
      points: '0,5 10,5 5,0'
    },
     2: {
      color: '#44a635',
      rvb: '68, 166, 53',
      cmjn: '82, 0, 100, 0',
      color_name: 'Vert pomme tout partout',
      titre_artwork: 'Les plus belles heures /\nSelf Contrôl, Allons',
      artiste: 'MC Mitout',
      date: 'Juillet 2016',
      story: 'Cette œuvre fait partie de la série <i>Les plus belles heures</i> présentant les meilleurs instants de la vie de l’artiste, MC Mitout. Le mot «Allons» s’adresse à celles et ceux qui le lisent et peut exprimer des émotions différentes. Cela encourage aussi à l’action, comme une invitation à parcourir l’exposition.',
      dapres_toi: 'D’après toi ?',
      dapres: [
        '« Allons enfants de la patrie »',
        '« Allons ramasser des petits pois »',
        '« Allons voir si la rose... » '
      ],
      images: [
        {
          src: "/images/chromo_6.jpg",
          credits: "© MC Mitout / Cnap. Photographie : Philippe Rolle."
        },
      ],
      points: '0,2.5 2.5,0 5,2.5 2.5,5'
    },
     3: {
      color: '#0b4ed1',
      rvb: '11, 78, 209',
      cmjn: '91, 67, 0, 0',
      color_name: 'Bleu plus bleu que le ciel bleu',
      titre_artwork: 'Photo-souvenir :\nRotation, travail in situ',
      artiste: 'Daniel Buren',
      date: 'Septembre 2006',
      story: 'Cette œuvre a été créée spécifiquement pour les ouvertures du musée. Le titre <i>Rotation</i> évoque la façon dont le motif de bandes verticales se déplace de fenêtre en fenêtre dans le sens des aiguilles d’une montre. Lorsque le soleil traverse les fenêtres, les couleurs des triangles sont projetées sur le sol et les murs du musée.',
      dapres_toi: 'D’après toi, dans la maison d’en face vit ?',
      dapres: [
        'Une vieille dame avec son chat',
        'Un fan de K-pop',
        'Une collectionneuse de mues de serpent'
      ],
      images: [
        {
          src: "/images/chromo_3.jpg",
          credits: "© DB – Adagp, Paris."
        },
      ],
      points: '0,10 0,0 5,5'
    },
    4: {
      color: '#1b3c75',
      rvb: '27, 60, 117',
      cmjn: '100, 80, 26, 9',
      color_name: 'Bleu foncé entre les lignes',
      titre_artwork: 'La Cabina',
      artiste: 'Nathalie Du Pasquier',
      date: '2021',
      story: 'Cabania de l’artiste Nathalie Du Pasquier est une œuvre dans laquelle les visiteur·euses peuvent entrer. Elle est envahie d’œuvres réalisées par d’autres artistes tel un cabinet de curiosités. Ici, ce cabinet est imaginé autour d’un animal : l’oiseau.',
      dapres_toi: 'D’après toi, quel est le meilleur endroit pour installer ce carrelage ?',
      dapres: [
        'La salle de bain de',
        'Le mobil-home de',
        'la piscine de'
      ],
      images: [
        {
          src: "/images/chromo_7.jpg",
          credits: "© Nathalie du Pasquier. Photographie : Aurélien Mole."
        },
      ],
      points: '5,0 0,0 2.5,2.5'
    },
    5: {
      color: '#ccffff',
      rvb: '204, 255, 255',
      cmjn: '27, 0, 0, 0',
      color_name: 'Bleu pâlichon tout autour',
      titre_artwork: 'CDV - Sans titre',
      artiste: 'Stefan Rinck',
      date: 'Juillet 2018',
      story: 'L’artiste, Stefan Rinck, appelle ce petit personnage « M.Flamboyant ». Il est habillé de plumes et de chaussures à pompons. Il a une allure à la fois élégante et ridicule. Pour imaginer ce personnage, l’artiste s’est inspiré de costumes de théâtre et plus particulièrement d’un style qui date de l’époque du roi Louis XIII',
      dapres_toi: 'D’après toi, ce «Mr. Flamboyant» va ?',
      dapres: [
        'À son premier « date »',
        'Réclamer des bonbons pour Halloween',
        'Jouer dans un film d’horreur'
      ],
      images: [
        {
          src: "/images/chromo_1.jpg",
          credits: "© Stefan Rinck. Photographie : Galerie Semiose."
        },
      ],
      points: '7.5,2.5 2.5,2.5 0,0 5,0'
    },
    6: {
      color: '#7b77d4',
      rvb: '123, 119, 212',
      cmjn: '62, 56, 0, 0',
      color_name: 'Violet zigzag',
      titre_artwork: 'La tour Belair',
      artiste: 'Genêt Mayor',
      date: '2013',
      story: 'Cette tour de Genêt Mayor ressemble à un jeu de construction. Elle est composée de caisses de bois que l’artiste a récupérées et peintes à l’extérieur. Les couleurs changent selon le côté qu’on regarde mais en reprenant toujours ce motif en zigzag : comme des escaliers qui permettent de monter en haut de la tour. Ses ouvertures permettent également de regarder à l’intérieur et de voir ses fondations.',
      dapres_toi: 'D’après toi, cette tour est ?',
      dapres: [
        'Un kaléidoscope pour géants',
        'Un cours de géométrie (mais tu as tout oublié)',
        'Un arbre à chat psychédélique'
      ],
      images: [
        {
          src: "/images/chromo_4.jpg",
          credits: "© Genêt Mayor. Photographie : Aurélien Mole."
        },
      ],
      points: '2.5,5 2.5,0 0,2.5'
    },
    7: {
      color: '#ffe215',
      rvb: '53, 92, 100',
      cmjn: '1, 8, 100, 0',
      color_name: 'Jaune citron néon',
      titre_artwork: 'Photo-souvenir : La Cabane éclatée aux caissons lumineux colorés',
      artiste: 'Daniel Buren',
      date: 'Déc 1999 - Jan 2000',
      story: 'Cette œuvre, créée par Daniel Buren, est la star du musée. Ses couleurs et sa lumière attirent immédiatement l’attention. Les nuances de bleu et de vert qui habillent l’extérieur rappellent la nature, le ciel et l’eau, alors que le jaune et le rouge présents à l’intérieur procurent une sensation de chaleur et de confort lorsqu’on se trouve entre les quatre murs de la cabane.',
      dapres_toi: 'D’après toi, cette cabane est la maison ?',
      dapres: [
        'De l’inventeur·ice du Rubik’s Cube',
        'D’un·e fan de Minecraft',
        'Du directeur du musée'
      ],
      images: [
        {
          src: "/images/chromo_2.jpg",
          credits: "© DB – Adagp, Paris. Photographie Jean-Paul Planchon."
        },
      ],
      points: '0,0 5,0 5,5'
    },
  };